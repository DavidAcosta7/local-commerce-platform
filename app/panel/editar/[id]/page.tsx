...
export default async function EditarComercioPage({ params }: PageProps) {
  ...
  // Fetch merchant and verify ownership
  const { data: merchant } = await supabase.from("merchants").select("*").eq("id", id).eq("owner_id", user.id).single()
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-orange-50">
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Editar Comercio</h1>
          <p className="text-gray-600">Actualiza la información de tu negocio</p>
        </div>

        <MerchantForm userId={user.id} merchant={merchant} />
      </div>
    </div>
  )
}
